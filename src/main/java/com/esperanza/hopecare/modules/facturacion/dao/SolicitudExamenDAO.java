...
public class SolicitudExamenDAO {
    public List<Object[]> listarNoFacturadasPorPaciente(int idPaciente, Connection conn) throws SQLException {
        String sql = "SELECT s.id_solicitud, e.precio FROM solicitud_examen s JOIN examen_laboratorio e ON s.id_examen = e.id_examen JOIN consulta c ON s.id_consulta = c.id_consulta JOIN cita ci ON c.id_cita = ci.id_cita WHERE ci.id_paciente = ? AND ci.estado = 'ATENDIDA' AND s.facturado = 0";
